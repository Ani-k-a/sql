...
-- SELECT avg(salary) from employees where first_name = 'Steven';
...
-- end) as "Salary Sum" FROM employees;
...
case
when salary < 10000 then 0 else 1
end
) as "are_enyone_more_10k" from employees group by department_id;
*/


-- вывести список из 2 колонок: department_id и число сотрудников в этом департаменте
-- select department_id, count(employee_id) as "summ_employes" from employees group by department_id;
...
-- SELECT sum(salary * 0.1) as "10%" from employees;


-- все сотрудники жертвуют 10% от зп в фонд защиты дикой природы. 
-- Посчитать сумму всех пожертвований для каждого департамента.
-- SELECT department_id, sum(salary * 0.1) as "10%" from employees group by department_id;


-- все сотрудники, зарабатывающие больше 10 000, жертвуют 10% от зп в фонд защиты дикой природы. 
-- Посчитать сумму всех пожертвований.
-- select sum(
-- case
-- when salary > 10000 then salary * 0.1
-- end
-- ) as "counted fine" from employees;

-- прилетел дракон и оштрафовал всех сотрудников на 10 000 из их зарплаты. 
-- Сотрудник не может получать отрицательное число денег, поэтому если зп сотрудника < 10000, то он штрафуется на 
-- величину своей зп. Посчитать сумму всех штрафов
-- select sum(
-- case
-- when salary < 10000 then salary else 10000
-- end
